...
from .ur5e_xhand_config import UR5eXHandConfig

logger = logging.getLogger(__name__)


class UR5eXHand(Robot):
    """
    Composite robot combining a 6-DOF UR5e arm with a 12-DOF XHand.
    """

    config_class = UR5eXHandConfig
    name = "ur5e_xhand"

    def __init__(self, config: UR5eXHandConfig):
    ...
    def connect(self, calibrate: bool = True) -> None:
        ...
        logger.info("Connecting UR5e + XHand composite robot...")
        try:
            ...
            logger.error("Failed to connect UR5e + XHand: %s", exc)
            try:
                ...
                logger.debug("Cleanup after failed UR5e + XHand connection failed", exc_info=True)
            raise ConnectionError(f"Failed to connect UR5e + XHand: {exc}") from exc
    ...
    def disconnect(self) -> None:
        ...
        try:
            ...
            logger.error("Failed to disconnect UR5e arm: %s", exc)
            arm_success = False

        try:
            self.hand.disconnect()
        except Exception as exc:
            logger.error("Failed to disconnect XHand: %s", exc)
            hand_success = False
        ...
        if arm_success and hand_success:
            logger.info("%s disconnected successfully", self)
        else:
            logger.warning("%s disconnected with errors", self)
