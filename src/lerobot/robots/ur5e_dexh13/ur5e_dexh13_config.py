...
@RobotConfig.register_subclass("ur5e_dexh13")
@dataclass
class UR5eDexH13Config(RobotConfig):
    """Configuration for a combined UR5e arm + DexH13 hand robot."""

    arm_config: UR5eConfig = field(default_factory=lambda: UR5eConfig(cameras={}))
    hand_config: DexH13Config = field(default_factory=lambda: DexH13Config(cameras={}))
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.hand_config, DexH13Config):
            raise TypeError("hand_config must be a DexH13Config")
